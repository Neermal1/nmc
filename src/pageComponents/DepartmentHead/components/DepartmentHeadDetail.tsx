...
const DepartmentHeadDetail = () => {
  ...
  const { fetchedData, loading, refetchData } = useFetchData(
    `departments/${departmentHead}/details`
  );
  ...
  useEffect(() => {
    if (router.isReady) {
      const { department_head } = router.query;
      setDepartmentHead(department_head);
    }
  }, [router]);
        ...
        <div className="layout component-padding ">
          <div className="flex flex-col gap-20">
                  ...
                  <div className="flex flex-col gap-10">
                    ...
                    <div className="flex flex-col gap-6">
                      <div className="lg:text-[35px] text-[25px] font-semibold">
                        {fetchedData?.details?.name}
                      </div>
                      <div className="leading-[30px]">
                        {fetchedData?.details?.description}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-col gap-10">
              ...
              <DepartmentBranch branchData={fetchedData?.departments} />
            </div>
          </div>
        </div>
      </div>
